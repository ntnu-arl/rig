...
/**
 * PreintegratedCombinedMeasurements2 integrates the IMU measurements
 ...
 * @ingroup navigation
 */
class GTSAM_EXPORT PreintegratedCombinedMeasurements2 : public PreintegrationType
{
public:
...
protected:
   ...
   * COVARIANCE OF: [PreintROTATION PreintVELOCITY BiasAcc BiasOmega]
   * (first-order propagation from *measurementCovariance*).
   * PreintegratedCombinedMeasurements2 also include the biases and keep the correlation
   * between the preintegrated measurements and the biases
   */
  Eigen::Matrix<double, 12, 12> preintMeasCov_;

  friend class CombinedImuFactor2;
...
public:
  ...
  PreintegratedCombinedMeasurements2() { preintMeasCov_.setZero(); }
   ...
   */
  PreintegratedCombinedMeasurements2(
  ...
  */
  PreintegratedCombinedMeasurements2(
    const PreintegrationType & base, const Eigen::Matrix<double, 12, 12> & preintMeasCov)
  : PreintegrationType(base), preintMeasCov_(preintMeasCov)
  {
  }

  /// Virtual destructor
  ~PreintegratedCombinedMeasurements2() override {}
  ...
  /// Re-initialize PreintegratedCombinedMeasurements2
  ...
  /// equals
  bool equals(const PreintegratedCombinedMeasurements2 & expected, double tol = 1e-9) const;
  /// @}
  ...
  void integrateMeasurement(
    const Vector3 & measuredAcc, const Vector3 & measuredOmega, const double dt) override;
 ...
 * 3) The covariance matrix of the PreintegratedCombinedMeasurements2 preserves
 *    the correlation between the bias uncertainty and the preintegrated
 *    measurements uncertainty.
 *
 * @ingroup navigation
 */
class GTSAM_EXPORT CombinedImuFactor2
: public NoiseModelFactorN<
    Rot3, Vector3, Rot3, Vector3, imuBias::ConstantBias, imuBias::ConstantBias>
{
public:
private:
  typedef CombinedImuFactor2 This;
  typedef NoiseModelFactorN<
    Rot3, Vector3, Rot3, Vector3, imuBias::ConstantBias, imuBias::ConstantBias>
    Base;

  PreintegratedCombinedMeasurements2 _PIM_;

public:
  /** Shorthand for a smart pointer to a factor */
#if !defined(_MSC_VER) && __GNUC__ == 4 && __GNUC_MINOR__ > 5
  typedef typename boost::shared_ptr<CombinedImuFactor2> shared_ptr;
#else
  typedef boost::shared_ptr<CombinedImuFactor2> shared_ptr;
#endif
   ...
   * @param PreintegratedCombinedMeasurements2 Combined IMU measurements
   */
  CombinedImuFactor2(
    Key rot_i, Key vel_i, Key rot_j, Key vel_j, Key bias_i, Key bias_j,
    const PreintegratedCombinedMeasurements2 & preintegratedMeasurements);

  ~CombinedImuFactor2() override {}
  ...
  /// @name Testable
  /// @{
  GTSAM_EXPORT friend std::ostream & operator<<(std::ostream & os, const CombinedImuFactor2 &);
  /// print
  ...
  const PreintegratedCombinedMeasurements2 & preintegratedMeasurements() const { return _PIM_; }

  /** implement functions needed to derive from Factor */

  /// vector of errors
  Vector evaluateError(
    const Rot3 & rot_i, const Vector3 & vel_i, const Rot3 & rot_j, const Vector3 & vel_j,
...
template <>
struct traits<PreintegratedCombinedMeasurements2>
: public Testable<PreintegratedCombinedMeasurements2>
{
};

template <>
struct traits<CombinedImuFactor2> : public Testable<CombinedImuFactor2>
{
};
