...
// Adapted from gtsam 4.2a9 CombinedImuFactor: removed position integration
...
//------------------------------------------------------------------------------
// Inner class PreintegratedCombinedMeasurements2
//------------------------------------------------------------------------------
void PreintegratedCombinedMeasurements2::print(const string& s) const {
  PreintegrationType::print(s);
  cout << "  preintMeasCov [ " << preintMeasCov_ << " ]" << endl;
}

//------------------------------------------------------------------------------
bool PreintegratedCombinedMeasurements2::equals(
    const PreintegratedCombinedMeasurements2& other, double tol) const {
  return PreintegrationType::equals(other, tol)
      && equal_with_abs_tol(preintMeasCov_, other.preintMeasCov_, tol);
}

//------------------------------------------------------------------------------
void PreintegratedCombinedMeasurements2::resetIntegration() {
...
#define D_R_v(H) (H)->block<3,3>(0,3)
// #define D_t_R(H) (H)->block<3,3>(3,0)
// #define D_t_t(H) (H)->block<3,3>(3,3)
// #define D_t_v(H) (H)->block<3,3>(3,6)
#define D_v_R(H) (H)->block<3,3>(3,0)
// #define D_v_t(H) (H)->block<3,3>(6,3)
#define D_v_v(H) (H)->block<3,3>(3,3)
#define D_a_a(H) (H)->block<3,3>(6,6)
#define D_g_g(H) (H)->block<3,3>(9,9)

//------------------------------------------------------------------------------
void PreintegratedCombinedMeasurements2::integrateMeasurement(
  ...
  // F_R_v
  F.block<3, 3>(0, 3) = A.block<3, 3>(0, 6);
  // F_v_R
  F.block<3, 3>(3, 0) = A.block<3, 3>(6, 0);
  // F_v_v
  F.block<3, 3>(3, 3) = A.block<3, 3>(6, 6);
  F.block<3, 3>(0, 9) = theta_H_biasOmega;
  // F.block<3, 3>(3, 9) = pos_H_biasAcc;
  F.block<3, 3>(3, 6) = vel_H_biasAcc;
  F.block<6, 6>(6, 6) = I_6x6;
  ...
  Eigen::Matrix<double, 12, 12> G_measCov_Gt;
  G_measCov_Gt.setZero(12, 12);
...
//------------------------------------------------------------------------------
// CombinedImuFactor2 methods
//------------------------------------------------------------------------------
CombinedImuFactor2::CombinedImuFactor2(Key rot_i, Key vel_i, Key rot_j,
    Key vel_j, Key bias_i, Key bias_j,
    const PreintegratedCombinedMeasurements2& pim) :
    Base(noiseModel::Gaussian::Covariance(pim.preintMeasCov_), rot_i, vel_i,
        rot_j, vel_j, bias_i, bias_j), _PIM_(pim) {
}

//------------------------------------------------------------------------------
gtsam::NonlinearFactor::shared_ptr CombinedImuFactor2::clone() const {
  return boost::static_pointer_cast<gtsam::NonlinearFactor>(
      gtsam::NonlinearFactor::shared_ptr(new This(*this)));
}

//------------------------------------------------------------------------------
void CombinedImuFactor2::print(const string& s,
...
//------------------------------------------------------------------------------
bool CombinedImuFactor2::equals(const NonlinearFactor& other, double tol) const {
  const This* e = dynamic_cast<const This*>(&other);
  return e != nullptr && Base::equals(*e, tol) && _PIM_.equals(e->_PIM_, tol);
}

//------------------------------------------------------------------------------
Vector CombinedImuFactor2::evaluateError(const Rot3& rot_i,
    const Vector3& vel_i, const Rot3& rot_j, const Vector3& vel_j,
  ...
  gtsam::Pose3 pose_i(rot_i, gtsam::Vector3::Zero());
  gtsam::Pose3 pose_j(rot_j, gtsam::Vector3::Zero());
  ...
  Vector6 r_Rv;
  r_Rv << r_Rpv.block<3, 1>(0, 0), r_Rpv.block<3, 1>(6, 0);

  // if we need the jacobians
  if (H1) {
    H1->resize(12, 3);
    H1->block<3, 3>(0, 0) = D_r_pose_i.block<3, 3>(0, 0);
    H1->block<3, 3>(3, 0) = D_r_pose_i.block<3, 3>(6, 0);
    // adding: [dBiasAcc/dPi ; dBiasOmega/dPi]
    H1->block<6, 3>(6, 0).setZero();
  }
  if (H2) {
    H2->resize(12, 3);
    H2->block<3, 3>(0, 0) = D_r_vel_i.block<3, 3>(0, 0);
    H2->block<3, 3>(3, 0) = D_r_vel_i.block<3, 3>(6, 0);
    // adding: [dBiasAcc/dVi ; dBiasOmega/dVi]
    H2->block<6, 3>(6, 0).setZero();
  }
  if (H3) {
    H3->resize(12, 3);
    H3->block<3, 3>(0, 0) = D_r_pose_j.block<3, 3>(0, 0);
    H3->block<3, 3>(3, 0) = D_r_pose_j.block<3, 3>(6, 0);
    // adding: [dBiasAcc/dPj ; dBiasOmega/dPj]
    H3->block<6, 3>(6, 0).setZero();
  }
  if (H4) {
    H4->resize(12, 3);
    H4->block<3, 3>(0, 0) = D_r_vel_j.block<3, 3>(0, 0);
    H4->block<3, 3>(3, 0) = D_r_vel_j.block<3, 3>(6, 0);
    // adding: [dBiasAcc/dVi ; dBiasOmega/dVi]
    H4->block<6, 3>(6, 0).setZero();
  }
  if (H5) {
    H5->resize(12, 6);
    H5->block<3, 6>(0, 0) = D_r_bias_i.block<3, 6>(0, 0);
    H5->block<3, 6>(3, 0) = D_r_bias_i.block<3, 6>(6, 0);
    // adding: [dBiasAcc/dBias_i ; dBiasOmega/dBias_i]
    H5->block<6, 6>(6, 0) = Hbias_i;
  }
  if (H6) {
    H6->resize(12, 6);
    H6->block<6, 6>(0, 0).setZero();
    // adding: [dBiasAcc/dBias_j ; dBiasOmega/dBias_j]
    H6->block<6, 6>(6, 0) = Hbias_j;
  }

  // overall error
  Vector r(12);
  r << r_Rv, fbias; // vector of size 12
  return r;
}

//------------------------------------------------------------------------------
std::ostream& operator<<(std::ostream& os, const CombinedImuFactor2& f) {
